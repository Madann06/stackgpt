...
const PDFUploadModal = ({ isOpen, onClose, onDocumentSelected }) => {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md p-2 sm:p-4 flex items-end sm:items-center justify-center">
        <motion.div
          initial={{ opacity: 0, y: 50, scale: 0.95 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          exit={{ opacity: 0, y: 50, scale: 0.95 }}
          className="bg-[#1E293B] border border-slate-700/80 rounded-t-2xl sm:rounded-2xl max-w-2xl w-full p-4 sm:p-6 shadow-2xl space-y-4 sm:space-y-6 relative max-h-[92vh] overflow-y-auto"
        >
          {/* Close Button */}
          <button
            type="button"
            ...
            onClick={onClose}
            className="absolute top-3 right-3 sm:top-4 sm:right-4 p-2.5 text-slate-400 hover:text-white rounded-xl bg-white/5 hover:bg-slate-800 min-h-[44px] min-w-[44px] flex items-center justify-center"
          >
            <X className="w-5 h-5" />
          </button>
