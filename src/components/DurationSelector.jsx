...
const DurationSelector = ({ selectedDuration, onSelectDuration, onAnalyze, isAnalyzing, symbol }) => {
  ...
  return (
    <motion.div
      ...
      className="glass-card rounded-2xl p-4 sm:p-6 border border-slate-700/60 shadow-2xl space-y-4 sm:space-y-6 relative overflow-hidden bg-slate-900/80 backdrop-blur-md"
    >
      {/* Top Banner Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 sm:pb-4 border-b border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-sm sm:text-base font-bold text-slate-100 flex items-center gap-2 flex-wrap">
              ...
              {symbol && <span className="text-[11px] font-mono text-blue-400 bg-blue-500/10 px-2 py-0.5 rounded border border-blue-500/30">${symbol}</span>}
            </h2>
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto">
          <span className="text-[10px] sm:text-[11px] font-mono text-amber-400 bg-amber-500/10 border border-amber-500/30 px-2.5 py-1 rounded-lg flex items-center gap-1.5">
            <ShieldAlert className="w-3.5 h-3.5" /> Required
          </span>
        </div>
      </div>

      {/* Duration Choice Track - Horizontal scroll on mobile, Grid on tablet/desktop */}
      <div className="flex sm:grid sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-2.5 sm:gap-3 overflow-x-auto hide-scrollbar pb-2 sm:pb-0 -mx-1 px-1">
        {DURATION_OPTIONS.map((opt) => {
          ...
          return (
            <button
              ...
              className={`p-3 rounded-xl border text-left transition-all relative flex flex-col justify-between shrink-0 w-36 sm:w-auto min-h-[44px] group ${
                isSelected
            ...
            >
              ...
              <p className={`text-[9px] sm:text-[10px] mt-1.5 line-clamp-2 leading-tight ${isSelected ? 'text-slate-200' : 'text-slate-500 group-hover:text-slate-400'}`}>
                {opt.focus}
              </p>
            </button>
          );
      ...
      <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-slate-800/80">
        <div className="text-xs text-slate-400 flex items-center gap-2 text-center sm:text-left">
          ...
          {activeSelection ? (
            ...
            <span className="text-amber-400 text-xs">Choose a duration above to analyze stock.</span>
          )}
        </div>
        ...
        <button
          ...
          className={`w-full sm:w-auto px-6 py-3 rounded-xl font-bold text-xs tracking-wider uppercase transition-all flex items-center justify-center gap-2 shadow-xl min-h-[44px] ${
